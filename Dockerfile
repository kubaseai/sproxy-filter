...
echo 'DebugLevel 3' >> /etc/c-icap/c-icap.conf && \
...
touch /var/log/clamd.log && chown clamscan:clamscan /var/log/clamd.log && \
...
HEALTHCHECK --start-period=90s CMD http_proxy=127.0.0.1:8443 curl -f http://www.msftncsi.com/ncsi.txt || exit 1
